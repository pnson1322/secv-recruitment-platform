...
type Props = {
  ...
  onPass?: (application: Application) => void;
  onReject?: (application: Application) => void;
  onViewCV?: (application: Application) => void;
};

export default function CandidateTableApproved({
  applications,
  onPass,
  onReject,
  onViewCV,
  ...
  if (applications.length === 0) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white px-6 py-16 text-center shadow-sm">
        <p className="text-[15px] text-slate-400">Không có ứng viên nào đã duyệt</p>
      </div>
    );
  }
